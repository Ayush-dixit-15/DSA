...
int firstOcuurance(int arr[], int size, int key){
    ...
    while (start<=end)
    {
        if (arr[mid] == key ){
            ...
            // we do this in order to make sure that we remain in the left most part of the array , we store the answer that is the mid's position and if get something lesser than that, we will get 
        }
        else if(arr[mid] < key){
            start = mid+1;
        }
