...
Node* temp = isCycle(head);
cout<<"earlier the value was"<<temp->next->data<<endl;
temp->next = NULL;
cout<<"now it is "<<temp->next->data<<endl;
}
...
tail->next = head->next;
// print(head);
cout<<"the loop os starting at"<<""<<getStarting(head)->data;
RemoveCycle(head)
return 0;
}